...
**本规范受 PHILOSOPHY.md（L5 绝对底层）约束。一切规范与哲学层冲突时，哲学层胜出。**
...
### L5 — 哲学层 (Philosophical Foundation)
- **生命周期**：永久，用户显式确认才可修改
- **存储位置**：`PHILOSOPHY.md`
- **内容**：广义指导思想（八条原则）— 矛盾论、执行谋略、实事求是参谋路线、实践真理观、持久战与根据地、自我批判与整风、环境的战争、认知纪律
- **更新规则**：仅用户显式要求时修改
- **容量控制**：八条原则，不可增减（除非用户要求）
- **约束范围**：所有下层（L4-L0）的全部规范、经验、行为最终受本层约束
...
### 7.0 哲学层强制加载
- PHILOSOPHY.md (L5) 在每次会话启动时**无条件加载**，先于一切节点
- 哲学层不参与防溢出折叠 — 永远展开
- 哲学层加载后，所有后续决策必须与六条原则核对一致性

### 7.1 加载策略
- **总是展开**：当前路径 (ROOT → BRANCH → 当前 LEAF) + PHILOSOPHY.md，最多 5 个系统节点
- **摘要加载**：已完成兄弟节点，仅标题 + 状态
- **按需加载**：非当前树的其他树，仅索引
...
```
加载节点时遵循:
1. PHILOSOPHY.md (L5) 永不折叠，不参与行数计数
2. 单次加载节点正文总行数 ≤ 200 行
3. 若当前路径超过 200 行 → 折叠最深层的 LEAF 节点为摘要
4. 若仍超过 → 逐层向上折叠直到满足
5. 优先折叠: COMPLETED > DORMANT > LOW priority
6. 绝不折叠: 当前活跃 LEAF, ABSOLUTE priority 节点, PHILOSOPHY.md
```
